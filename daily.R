...
setwd(paste0(basepath,"/repositories/invercargill2020/mapping/"))
data_path <- "./"
...
wanted_tag_human <- "invercargill2020"
...
# x_now <- as.POSIXct("2020-06-16 18:00:00")
print(x_now)
...
RCurl::ftpUpload(paste0(data_path,
                        ...
                        format(max(all_data.tavg$date) + 12*3600,format = "%Y%m%d"),
                        ".png"),
                 paste0("ftp://ftp.niwa.co.nz/incoming/GustavoOlivares/odin_invercargill/",
                        't_series_',
                        format(min(all_data.tavg$date) + 12*3600,format = "%Y%m%d"),"_",
                        format(max(all_data.tavg$date) + 12*3600,format = "%Y%m%d"),
                        ".png"))
...
# updateStatus("Data availability from invercargill - UTC time", mediaPath = paste0(data_path,
#                                                                                'availability_',
#                                                                                format(min(all_data.tavg$date) + 12*3600,format = "%Y%m%d"),"_",
#                                                                                format(max(all_data.tavg$date) + 12*3600,format = "%Y%m%d"),
#                                                                                ".png"))
# 
# updateStatus("Latest time series from invercargill - UTC time", mediaPath = paste0(data_path,
#                                                                                 't_series_',
#                                                                                 format(min(all_data.tavg$date) + 12*3600,format = "%Y%m%d"),"_",
#                                                                                 format(max(all_data.tavg$date) + 12*3600,format = "%Y%m%d"),
#                                                                                 ".png"))
...
if (location_ok){
  ...
  system(paste0("youtube-upload --title=\"invercargill ",
                ...
                format(min(all_data.tavg$date) + 12*3600,format = "%Y%m%d"),"_",
                format(max(all_data.tavg$date) + 12*3600,format = "%Y%m%d"),
                ".mp4 --playlist=\"invercargill 2020 - ODIN\""))
  ...
  #               format(min(all_data.tavg$date) + 12*3600,format = "%Y%m%d"),"_",
  #               format(max(all_data.tavg$date) + 12*3600,format = "%Y%m%d"),
  #               ".mp4 --playlist=\"invercargill 2020 - ODIN - AutoKrige\""))
  
  # Upload files
  print("Upload NC files")
  RCurl::ftpUpload(paste0(data_path,"odin_idw.nc"),
                   "ftp://ftp.niwa.co.nz/incoming/GustavoOlivares/odin_invercargill/odin_idw.nc")
  RCurl::ftpUpload(paste0(data_path,"odin_idw2.nc"),
                   "ftp://ftp.niwa.co.nz/incoming/GustavoOlivares/odin_invercargill/odin_idw2.nc")
...
#                                    "invercargill script finished OK:",
#                                    format(max(all_data.tavg$date) + 12*3600,format = "%Y%m%d")))
